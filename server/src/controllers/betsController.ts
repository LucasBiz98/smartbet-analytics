...
export const betsController = {
  ...
  async getBets(req: Request, res: Response, next: NextFunction) {
    try {
      const { 
        status, 
        limit = 50, 
      ...
      logger.error('Error en getBets', { error: (error as Error).message });
      next(error);
    }
  },
  
  // Obtener apuesta por ID
  async getBet(req: Request, res: Response, next: NextFunction) {
    try {
      const { id } = req.params;
      ...
      logger.error('Error en getBet', { error: (error as Error).message });
      next(error);
    }
  },
  ...
  async createBet(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      logger.info('Apuesta creada', { betId: result.rows[0].id, amount, oddsTaken });
      ...
      logger.error('Error en createBet', { error: (error as Error).message });
      next(error);
    }
  },
    ...
    try {
      ...
      if (status) {
        ...
        if (status === 'WON' || status === 'LOST') {
          ...
          const betResult = await pool.query('SELECT * FROM bets WHERE id = $1', [id]);
      ...
      logger.info('Apuesta actualizada', { betId: id, status });
      ...
      logger.error('Error en updateBet', { error: (error as Error).message });
      next(error);
    }
  },
  ...
  async deleteBet(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await pool.query('DELETE FROM bets WHERE id = $1 RETURNING id', [id]);
      
      if (result.rows.length === 0) {
        return res.status(404).json({ error: 'Apuesta no encontrada' });
      }
      
      logger.info('Apuesta eliminada', { betId: id });
      ...
      logger.error('Error en deleteBet', { error: (error as Error).message });
      next(error);
    }
  },
  
  // Obtener apuestas pendientes
  async getPendingBets(req: Request, res: Response, next: NextFunction) {
    try {
      const result = await pool.query(`
        SELECT 
          b.*,
      ...
      logger.error('Error en getPendingBets', { error: (error as Error).message });
      next(error);
    }
  }
};
