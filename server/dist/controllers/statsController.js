...
exports.statsController = {
    ...
    async getDashboardStats(req, res, next) {
        try {
            ...
            const generalStats = await database_1.pool.query(`
        SELECT 
          COUNT(*) as total_bets,
        ...
        FROM bets
        WHERE placed_at >= NOW() - INTERVAL '${parseInt(period)} days'
      `);
            // Calcular win rate
            ...
            const stakeStats = await database_1.pool.query(`
        SELECT 
          p.stake,
        ...
        AND b.placed_at >= NOW() - INTERVAL '${parseInt(period)} days'
        GROUP BY p.stake
        ORDER BY p.stake DESC
      `);
            // Últimas 10 apuestas
            const recentBets = await database_1.pool.query(`
        SELECT 
          b.*,
          m.home_team,
          m.away_team,
      ...
      `);
            ...
            const dailyStats = await database_1.pool.query(`
        SELECT 
      ...
      `);
            ...
            logger_1.logger.error('Error en getDashboardStats', { error: error.message });
            next(error);
        }
    },
    ...
    async getMarketStats(req, res, next) {
        ...
        FROM bets b
        WHERE b.placed_at >= NOW() - INTERVAL '${parseInt(period)} days'
        GROUP BY b.market
      ...
      `);
            ...
            logger_1.logger.error('Error en getMarketStats', { error: error.message });
            next(error);
        }
    },
    ...
    async getLeagueStats(req, res, next) {
      ...
      `);
            ...
            logger_1.logger.error('Error en getLeagueStats', { error: error.message });
            next(error);
        }
    },
    ...
    async getPerformanceHistory(req, res, next) {
        ...
        FROM bets
        WHERE placed_at >= NOW() - INTERVAL '${parseInt(days)} days'
      ...
      `);
            ...
            logger_1.logger.error('Error en getPerformanceHistory', { error: error.message });
            next(error);
        }
    }
};
